...
Test classes
------------
* ``TestAlbResponseS3``       – helper function ``alb_response``
...
class TestAlbResponseS3:
    ...
    def test_custom_headers_override_default(self):
        ...
        response = s3_lambda.alb_response(200, {}, headers=custom)
...
class TestHandleDryRunS3:
    ...
    def test_dry_run_response_includes_resource_name(self):
        response = s3_lambda.handle_dry_run_s3(ACCOUNT_ID, "present-bucket", "GET")
        body = json.loads(response["body"])
        assert body["resource_name"] == "present-bucket"
...
class TestLambdaHandlerS3:
    ...
    def test_missing_bucket_name_returns_400(self):
        event = {"path": f"/unlock-s3-bucket/{ACCOUNT_ID}"}
        response = s3_lambda.lambda_handler(event, None)
        assert response["statusCode"] == 400
        body = json.loads(response["body"])
        assert body["message"] == "Missing bucket_name in path parameters"

    def test_empty_path_returns_400(self):
        response = s3_lambda.lambda_handler({"path": ""}, None)
        assert response["statusCode"] == 400

    # --- protected bucket guard ---------------------------------------------

    def test_bucket_in_protected_list_returns_403(self):
        event = {"path": f"/unlock-s3-bucket/{ACCOUNT_ID}/protected-bucket"}
        with patch.object(s3_lambda, "PROTECTED_BUCKETS", ["protected-bucket"]):
            response = s3_lambda.lambda_handler(event, None)
        assert response["statusCode"] == 403
        body = json.loads(response["body"])
        assert "protected" in body["message"]

    def test_tf_state_pattern_bucket_returns_403(self):
        tf_bucket = f"{ACCOUNT_ID}-tf-state"
        event = {"path": f"/unlock-s3-bucket/{ACCOUNT_ID}/{tf_bucket}"}
    ...
    def test_get_returns_bucket_policy(
        ...
        assert body["policy"] == SAMPLE_S3_POLICY
